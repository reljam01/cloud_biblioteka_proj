...
void run_server(mongoc_collection_t *collection, int env_port) {
  ...
  while(true) {
	...
	recv(clientSocket, newdate, sizeof(newdate), 0);
    	...
    	int isbncount;
    	bson_t *isbnquery7 = bson_new();
    	BSON_APPEND_UTF8(isbnquery7, "isbn", isbn);
    	isbncount = mongoc_collection_count(collection,MONGOC_QUERY_NONE,isbnquery7,0,0,NULL,&error);
    	bson_destroy(isbnquery7);
	...
	recv(clientSocket, id, sizeof(id), 0);
    	...
    	if(isbncount == 0) {
  		...
  		printf("ISPRAVLJEN QUERY\n");
  		bson_t *isbnquery = BCON_NEW("isbn", BCON_UTF8(isbn),"clanski_broj",BCON_INT32(atoi(id)));
    		//BSON_APPEND_UTF8(isbnquery, "isbn", isbn);
